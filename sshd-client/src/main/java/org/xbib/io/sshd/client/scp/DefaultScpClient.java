...
import org.xbib.io.sshd.common.file.util.MockFileSystem;
import org.xbib.io.sshd.common.file.util.MockPath;
import org.xbib.io.sshd.common.scp.ScpFileOpener;
import org.xbib.io.sshd.common.scp.ScpHelper;
import org.xbib.io.sshd.common.scp.ScpLocation;
...
public class DefaultScpClient extends AbstractScpClient {
    ...
    public void download(String remote, OutputStream local) throws IOException {
        String cmd = org.xbib.io.sshd.client.scp.ScpClient.createReceiveCommand(remote, Collections.emptyList());
    ...
    protected void download(String remote, FileSystem fs, Path local, Collection<Option> options) throws IOException {
        String cmd = org.xbib.io.sshd.client.scp.ScpClient.createReceiveCommand(remote, options);
        ClientSession session = getClientSession();
        ChannelExec channel = openCommandChannel(session, cmd);
        try (InputStream invOut = channel.getInvertedOut();
             OutputStream invIn = channel.getInvertedIn()) {
    ...
    public void upload(final InputStream local, final String remote, final long size, final Collection<PosixFilePermission> perms, final ScpTimestamp time) throws IOException {
        ...
        final String cmd = org.xbib.io.sshd.client.scp.ScpClient.createSendCommand(remote, (time != null) ? EnumSet.of(Option.PreserveAttributes) : Collections.emptySet());
        ClientSession session = getClientSession();
        ChannelExec channel = openCommandChannel(session, cmd);
        try (InputStream invOut = channel.getInvertedOut();
             OutputStream invIn = channel.getInvertedIn()) {
    ...
    protected <T> void runUpload(String remote, Collection<Option> options, Collection<T> local, ScpOperationExecutor<T> executor) throws IOException {
        ...
        String cmd = org.xbib.io.sshd.client.scp.ScpClient.createSendCommand(remote, options);
        ClientSession session = getClientSession();
        ChannelExec channel = openCommandChannel(session, cmd);
        try {
