...
public class SftpCommand implements Channel {
    ...
    private abstract class TransferCommandExecutor implements CommandExecutor {
        ...
        protected void transferFile(SftpClient sftp, Path localPath, String remotePath, int bufferSize,
                                    boolean upload, PrintStream stdout, boolean verbose) throws IOException {
        ...
        protected void transferRemoteDir(SftpClient sftp, Path localPath, String remotePath, int bufferSize,
                                         Attributes attrs, PrintStream stdout, boolean verbose) throws IOException {
            if (attrs.isDirectory()) {
                for (DirEntry entry : sftp.readDir(remotePath)) {
                    ...
                    transferRemoteDir(sftp, localPath.resolve(name), remotePath + "/" + name,
                            bufferSize,
                            entry.getAttributes(), stdout, verbose);
                }
            } else if (attrs.isRegularFile()) {
                transferFile(sftp, localPath, remotePath, bufferSize,false, stdout, verbose);
            } else {
                if (verbose) {
        ...
        protected void transferLocalDir(SftpClient sftp, Path localPath, String remotePath, int bufferSize,
                                        PrintStream stdout, boolean verbose) throws IOException {
            if (Files.isDirectory(localPath)) {
                try (DirectoryStream<Path> ds = Files.newDirectoryStream(localPath)) {
                    for (Path entry : ds) {
                        String name = entry.getFileName().toString();
                        transferLocalDir(sftp, localPath.resolve(name), remotePath + "/" + name,
                                bufferSize, stdout, verbose);
                    }
                }
            } else if (Files.isRegularFile(localPath)) {
                transferFile(sftp, localPath, remotePath, bufferSize, true, stdout, verbose);
            } else {
                if (verbose) {
        ...
        protected void executeCommand(String args, int bufferSize, boolean upload, PrintStream stdout) throws IOException {
            String[] comps = GenericUtils.split(args, ' ');
            int numArgs = GenericUtils.length(comps);
            ...
            if (recursive) {
                if (upload) {
                    ...
                    transferLocalDir(sftp, local, remote, bufferSize, stdout, verbose);
                } else {
                    Attributes attrs = sftp.stat(remote);
                    ValidateUtils.checkTrue(attrs.isDirectory(), "Remote path not a directory: %s", remote);
                    transferRemoteDir(sftp, local, remote, bufferSize, attrs, stdout, verbose);
                }
            } else {
                ...
                transferFile(sftp, local, remote, bufferSize, upload, stdout, verbose);
            }
        }
    }
    ...
    private class GetCommandExecutor extends TransferCommandExecutor {
        ...
        @Override
        public boolean executeCommand(String args, BufferedReader stdin, PrintStream stdout, PrintStream stderr) throws Exception {
            executeCommand(args, IoUtils.DEFAULT_COPY_SIZE, false, stdout);
            return false;
        }
    }
    ...
    private class PutCommandExecutor extends TransferCommandExecutor {
        ...
        @Override
        public boolean executeCommand(String args, BufferedReader stdin, PrintStream stdout, PrintStream stderr) throws Exception {
            executeCommand(args, IoUtils.DEFAULT_COPY_SIZE, true, stdout);
            return false;
        }
    }
