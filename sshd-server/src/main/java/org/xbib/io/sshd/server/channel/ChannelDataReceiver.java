 ...
 * By default {@link org.xbib.io.sshd.server.channel.ChannelSession} spools this in a buffer so that you can read it from
 * the input stream you get from {@link org.xbib.io.sshd.server.Command#setInputStream(java.io.InputStream)}, but if command
 * wants to do a callback-driven I/O for the data it receives from the client, it can
 * call {@link org.xbib.io.sshd.server.channel.ChannelSession#setDataReceiver(ChannelDataReceiver)} to do so.
 * (And to grab a reference to {@link org.xbib.io.sshd.server.channel.ChannelSession}, a {@link org.xbib.io.sshd.server.Command} should implement
 * {@link org.xbib.io.sshd.server.ChannelSessionAware}.)
 */
